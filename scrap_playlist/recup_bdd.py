...
from pymongo import (
    MongoClient,
)  # création de notre collection qui va contenir les infos
...
def recup_playlist_from_channels(id_chaine):
    ...
    bdd = []
    lien = (
        "https://www.youtube.com/channel/{}/playlists?view=1&sort=dd&shelf_id=0".format(
            id_chaine
        )
    )
    driver.get(lien)
...
def get_playlist_data(url, id_chaine=None, id_playlist=None):
    ...
    if id_playlist is not None:
        ...
        id_playlist = re.findall(
            r"https:\/\/www\.youtube\.com\/playlist\?list=([a-zA-Z0-9-_]*)", url
        )[0]
    ...
    video_time = driver.find_elements_by_class_name(
    ...
    for l in video_links:
        ...
        if re.match(
            r"https://www\.youtube\.com/watch\?v=(\S+)&list=\S+&index=\d+", lien
        ):

            video_ids.append(
                re.findall(r"https://www\.youtube\.com/watch\?v=(\S+)&list=\S+", lien)[
                    0
                ]
            )
...
if __name__ == "__main__":
    ...
    for i in range(len(liste_ytbeurs)):
        ...
        nb_playlist_existante = len(
            list(col_playlist.find({"id_youtubeur": id_chaine}))
        )

        if nb_playlist_existante == 0:

            playlist_title, playlist_urls = recup_playlist_from_channels(
                liste_ytbeurs[i]
            )
